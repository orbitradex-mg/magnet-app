...
const createTables = async () => {
  // Используем нашу функцию run, которая правильно обрабатывает lastID
  ...
  const existingOrder = await getQuery('SELECT id FROM orders WHERE order_number = ?', ['1825']);
  if (!existingOrder) {
    const result = await runQuery('INSERT INTO orders (order_number, status) VALUES (?, ?)', 
      ['1825', 'in_progress']);
